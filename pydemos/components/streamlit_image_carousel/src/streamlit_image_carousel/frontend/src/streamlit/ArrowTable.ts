...
export class ArrowTable {
  ...
  constructor(
      dataBuffer: Uint8Array, indexBuffer: Uint8Array,
      columnsBuffer: Uint8Array, styler?: any) {
  ...
  getCell = (rowIndex: number, columnIndex: number): Cell => {
    ...
    if (isBlankCell) {
      ...
      const content = this.styler ?
          this.getContent(
              this.styler.displayValuesTable, dataRowIndex, dataColumnIndex) :
  ...
  getContent = (table: Table, rowIndex: number, columnIndex: number): any => {
